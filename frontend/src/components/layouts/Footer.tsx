...
const Footer = (props: FooterProps) => {
  ...
  return (
    <footer class="sticky bottom-0 z-50 h-[2rem] flex items-center border-t bg-gray-500/10 border-gray-800/50 justify-between px-4">
      <div class="flex items-center justify-start">
        <p>© 2025 {company.name}. All rights reserved.</p>

        <button onClick={handleScreenShot} title="Take a screenshot of the current screen" class="flex items-center justify-center gap-2 ml-6 cursor-pointer">
          <Icon icon="mdi:monitor-screenshot" width="20" height="20" />Screenshot
        </button>

        <button onClick={handleScreenRecordToggle} title="Screen Record" class="flex items-center justify-center gap-2 ml-6 cursor-pointer">
       
          <Icon icon={isRecording() ? 'mdi:stop-circle' : 'mdi:record-rec'} width="20" height="20" class={isRecording() ? 'text-red-500' : ''} />  {isRecording() ? 'Recording...' : 'Record Screen'}
        </button>
      </div>
    </footer>
  );
