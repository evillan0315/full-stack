        ...
        <div class={`${type()}-dialog-modal-body dialog-modal-body rounded-lg shadow-lg p-6 max-w-sm w-full space-y-4`}>
          ...
          <div class="flex justify-end gap-2">
            <Show when={type() !== 'alert'}>
              <Button variant="secondary" icon="mdi:close" size="sm" onClick={handleCancel}>
                Cancel
              </Button>
            </Show>
            <Button variant={resolveOkVariant()} size="sm" icon="mdi:check" class="px-4" onClick={handleOk}>
              OK
            </Button>
          </div>
        </div>
