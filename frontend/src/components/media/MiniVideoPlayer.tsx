...
export default function MiniVideoPlayer(props: MiniVideoPlayerProps) {
  ...
  const fetchVideos = async (directory = './downloads/videos', collected: any[] = []) => {
    try {
  ...
  const loadVideo = (index: number) => {
    ...
    player.src({ type: 'video/mp4', src: `${import.meta.env.BASE_URL_API}/api/media/videos/${vid.name}` });
    player.load();
