...
export default function CodeEditor() {
  const [dividerX, setDividerX] = createSignal(window.innerWidth / 4.4); // use window width immediately
  const [filePath, setFilePath] = createSignal('./README.md');
  const [fileContent, setFileContent] = createSignal<string>('');
  const [isLoading, setIsLoading] = createSignal(true); // added loading state
