...
export class JsDocToMarkdownController {
  ...
  @ApiQuery({ name: 'filePath', required: true, description: 'Path to the .ts or .tsx file' })
  @ApiResponse({ status: 200, description: 'Markdown documentation content as string.' })
  ...
  @ApiQuery({ name: 'filePath', required: true, description: 'Path to the source .ts or .tsx file' })
  @ApiQuery({ name: 'outputPath', required: true, description: 'Path where the Markdown file will be written' })
  @ApiResponse({ status: 200, description: 'Successfully exported Markdown file' })
  async exportMarkdown(
    @Query('filePath') filePath: string,
    @Query('outputPath') outputPath: string,
  ): Promise<{ message: string }> {
    if (!filePath || !outputPath) {
      throw new BadRequestException('Both filePath and outputPath are required');
    }
