...
export default function EditorBottomNav(): JSX.Element {
  ...
  const handleGenerateDocumentation = async () => {
    ...
    try {
      const currentContent = editorContent.get();
      const currentLanguage = language(); // Assuming you want the current language
      const payload = {
      ...
      setDocGeneratedContent(newContent);
      showToast('Documentation generated successfully.', 'success');
      setDocDrawerOpen(false); // Close the documentation drawer after success
    } catch (err: any) {
      showToast(`Error: ${err.message || 'Unknown error'}`, 'error');
  ...
  return (
    <>
      <div class="flex items-center justify-between gap-4 py-2">
          ...
          items={[
            {
              ...
              icon: 'mdi:code',
              onClick: () => setDocDrawerOpen(true), // Open documentation drawer
            },
            {
              label: 'Generate Code',
              icon: 'mdi:code',
        ...
        <Button variant="outline" onClick={toggleTerminal}>
